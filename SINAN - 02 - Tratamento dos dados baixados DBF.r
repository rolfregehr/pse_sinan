...
arq_sinan_dbf <- list.files('./dados_sinan/', full.names = T, pattern = 'dbf')
...
sinan_temp <- read.dbf(arq_sinan_dbf[1]) |> 
  select(all_of(colunas_selecionadas))

for(arq in arq_sinan_dbf[2:length(arq_sinan_dbf)]){
  print(arq)
  temp <- read.dbf(arq)|> 
    select(all_of(colunas_selecionadas))
